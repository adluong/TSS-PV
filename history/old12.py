...
import secrets, hashlib, time, random, gc, tracemalloc, starkcurve_py as cp
...
# ────────── curve wrappers ──────────
B, ID = map(lambda s: E(s, 33), (cp.generator(), cp.identity()))
P = lambda A, B: E(cp.point_addition(E(A, 33), E(B, 33)), 33)
Pm = lambda s, P_: bump() or E(cp.point_multiply(E(s), E(P_, 33)), 33)
Sm = lambda s: bump() or E(cp.scalar_multiply(E(s)), 33)
...
ONE = b'\x00'*31 + b'\x01'
H1, H2 = (Pm(b'\x00'*31 + bytes([i]), B) for i in (2, 3))
...
def recon(shs):
    acc = E(cp.scalar_multiply(b"\0" * 32), 33)  # ID
    k = len(shs)
...
def Rbox(k, embeds):
    def R(extra):
        ...
        uniq = {}
        [uniq.setdefault(x, y) for x, y in embeds + extra]
        if len(uniq) < k:
            return None
...
def run(n, k, f):
    ...
    rb_reset()
    globals().__setitem__("RB_TIME", 0.0)
...
if __name__ == "__main__":
    for n, k, f in [(64, 33, 10), (128, 65, 20), (256, 129, 40)]:
        run(n, k, f)