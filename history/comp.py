...
def reconstruct(shares: list[tuple[bytes, bytes]]) -> bytes:
    ...
    for j in range(k):
        ...
        print(f" - Shares {j}: {xj_bytes.hex()},{yj_bytes.hex()}\n") 
...
def zkpok_proof_one1(h_value: bytes, h1: bytes, h2: bytes, ys: list[bytes], r: bytes, s: bytes) -> tuple[bytes, bytes, bytes]:
    # Sample random scalars y and z as 32-byte objects
    ...
    # Compute commitments
    fk1 = ensure_bytes(cp.point_multiply(k1, h_value))
    gk2 = ensure_bytes(cp.point_multiply(k2))
    h1k1 = ensure_bytes(cp.point_multiply(k1, h1))
    h2k2 = ensure_bytes(cp.point_multiply(k1, h2))
    commitment = ensure_bytes(cp.point_addition(fk1, gk2))
    commitment2 = ensure_bytes(cp.point_addition(h1k1, h2k2))
    commitments.append(commitment)
    commitments.append(commitment2)

    # Create hash input by concatenating f_values and commitments as bytes
    hash_data = b"".join(ys + commitments)
    # Compute SHA-256 hash and truncate to 32 bytes for scalar compatibility
    hash_value = hashlib.sha256(hash_data).digest()[:32]

    # Compute u = y - (hash_value * r)
    hash_r = ensure_bytes(cp.scalar_multiply_scalar(hash_value, r))
    u = ensure_bytes(cp.scalar_subtraction(k1, hash_r))

    # Compute v = z - (hash_value * s)
    hash_s = ensure_bytes(cp.scalar_multiply_scalar(hash_value, s))
    v = ensure_bytes(cp.scalar_subtraction(k2, hash_s))

    return (hash_value, u, v)
...
def generate_shares_original(secret_scalar: bytes, n: int, k: int) -> list[tuple[bytes, bytes]]:
    """
    Generate n shares for a (k, n)-threshold Shamir's Secret Sharing scheme.

    Args:
        secret_scalar: The secret scalar as a 32-byte string.
        n: Total number of shares (e.g., 30).
        k: Threshold number of shares required (e.g., 20).

    Returns:
        List of tuples (x_i, y_i), where x_i is a scalar (bytes) and y_i is a point (bytes).
    """
    ...
    for x in x_values:
        ...
        for coef in coefficients[1:]:
            ...
            x_pow = ensure_bytes(cp.scalar_multiply_scalar(x_pow, x))
        y_point = ensure_bytes(cp.scalar_multiply(y))
        shares.append((x, y_point))
    return shares
...
def test_reconstruction():
    ...
    print(f"Secret to be shared (original point): {original_secret.hex()}")
    # n, k = 30, 20
    shares = generate_shares_original(secret_scalar, n, k)
    print(f"Generated {n} shares with threshold {k}.")
    selected_shares = shares[:k]
    reconstructed_secret = reconstruct(selected_shares)
    print(f"Reconstructed secret from {k} shares: {reconstructed_secret.hex()}")
    if reconstructed_secret == original_secret:
        print("Test passed: Reconstructed secret matches the original secret.")
    else:
        print("Test failed: Reconstructed secret does not match the original secret.")
